    ...
    <div className='bg-[#02013D] w-[43%] pl-[5.3%] pt-14 pb-12 relative flex flex-col gap-4 h-full xsm:w-full xsm:h-[65vh]'>
      ...
      <div className='overflow-y-auto space-y-4 noScroll z-[20]'>
        {trips.map((trip, index) => (
          ...
          >
            <div className='bg-[#941A21B5] flex items-center px-4 py-1 w-max xsm:px-2 xsm:py-[2px]'>
              <p className='text-[24px] text-white font-[Merriweather-Sans] xsm:text-[15px]'>{trip.tripsCount}</p>
            </div>
            <div className='text-white pb-4 pl-6 flex flex-col gap-2 CU-Trip-bg-custom-gradient xsm:gap-[2px] xsm:pl-3 xsm:pb-2'>
              <p className='font-[Merriweather] text-[22px] font-semibold tracking-[0.05em] xsm:text-[14px]'>{trip.location}</p>
              <p className='font-[Merriweather-Sans] font-thin uppercase text-[16px] tracking-[0.05em] xsm:text-[10px]'>{trip.subtitle}</p>
            </div>
          </div>
        ))}
      </div>
