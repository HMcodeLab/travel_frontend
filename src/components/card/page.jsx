    ...
    <div className='rounded-lg flex flex-col shadow-lg bg-[#F4F4F4] pb-5'>
      <div className='relative w-full h-[33vh]'>
        ...
        <div className='w-full flex justify-between absolute top-0 text-white p-3'>
          <span className='flex items-center bg-black px-2 rounded gap-1'>
            {val?.days} <Image src={Moon} alt='Moon icon' width={12} height={12} /> / {val?.night} <Image src={Sun} alt='Sun icon' width={12} height={12} />
          </span>
      ...
      <div className='flex flex-col gap-4 px-3'>
        <div className='flex gap-3 items-center mt-3 font-int min-h-10'>
          {val?.servies?.map((item, index) => (
            <div key={index} className='flex items-center gap-1 text-xs font-semibold'>
        ...
        <div className='w-full flex justify-between items-center' style={{ fontFamily: 'Montserrat' }}>
          <div className='flex items-center gap-1'>
            ...
            <strike className="text-[#5F5D5D] text-sm font-semibold">₹{val?.discount || 200}/-</strike>
          </div>
          <div className='flex items-center gap-1'>
